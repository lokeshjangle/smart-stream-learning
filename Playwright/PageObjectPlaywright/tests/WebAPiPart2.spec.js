//Handling saved session/cookies for login
...
test.beforeAll(async function ({ browser }) {
  ...
  await context.storageState({ path: 'state.json' }); //this is create for whole brower

  //openinig new context
...
test('Browser Context-Validating Error Login', async () => {
  ...
  const email = 'lokeshjangle@gmail.com';
  const page = await webContext.newPage(); //Page is created dynamically so no need to page fixture
  await page.goto('https://rahulshettyacademy.com/client');
  ...
  await expect(productVisible).toBeTruthy();
